...
class LivraisonController extends Controller
{
    public function toutesLivraisons(Request $request)
    {
        ...
        $dayBase = Commande::where('statut', 'validee')->where('type_commande', 'livraison');
        if ($boutiqueId) $dayBase->where('boutique_id', $boutiqueId);
        $dayBase->whereDate('date_commande', $date);

        $total = (clone $dayBase)->count();
        $enAttente = (clone $dayBase)->where(function ($q) { $q->whereNull('statut_livraison')->orWhere('statut_livraison', 'en_attente'); })->count();
        $assignees = (clone $dayBase)->where('statut_livraison', 'assignee')->count();
        $livrees = (clone $dayBase)->where('statut_livraison', 'livree')->count();

        $enRetardQuery = Commande::where('statut', 'validee')->where('type_commande', 'livraison')
            ->whereDate('date_commande', '<', $today)
            ->where(function ($q) { $q->whereNull('statut_livraison')->orWhere('statut_livraison', 'en_attente')->orWhere('statut_livraison', 'assignee'); });
        ...
        if ($statutFilter === 'en_retard') {
            $commandes = Commande::with(['client', 'employe', 'livreur', 'boutique', 'produits'])
                ->where('statut', 'validee')->where('type_commande', 'livraison')
                ->whereDate('date_commande', '<', $today)
                ->where(function ($q) { $q->whereNull('statut_livraison')->orWhere('statut_livraison', 'en_attente')->orWhere('statut_livraison', 'assignee'); });
            ...
            if ($statutFilter === 'en_attente') {
                $commandes->where(function ($q) { $q->whereNull('statut_livraison')->orWhere('statut_livraison', 'en_attente'); });
            } elseif ($statutFilter) {
        ...
        return response()->json([
            'resume' => ['total' => $total, 'en_attente' => $enAttente, 'assignees' => $assignees, 'livrees' => $livrees, 'en_retard' => $enRetardCount],
            'commandes' => $commandes,
        ]);
    }
    ...
    public function assigner(Request $request, Commande $commande)
    {
        ...
        if ($commande->type_commande !== 'livraison') return response()->json(['message' => 'Cette commande n\'est pas une livraison'], 400);
        if ($commande->statut !== 'validee') return response()->json(['message' => 'La commande doit être validée'], 400);
    ...
    {
        $user = $request->user();
        if (!($user instanceof Livreur)) return response()->json(['message' => 'Accès réservé aux livreurs'], 403);
        ...
        $aLivrer = Commande::with(['client', 'employe', 'boutique', 'produits'])
            ...
            ->whereDate('date_commande', $date)->orderBy('created_at', 'desc')->get();

        $livrees = Commande::with(['client', 'employe', 'boutique', 'produits'])
            ->where('livreur_id', $user->id)->where('statut_livraison', 'livree')
            ->whereDate('date_livraison', $date)->orderBy('date_livraison', 'desc')->get();
    ...
    {
        $user = $request->user();
        if ($user instanceof Livreur && $commande->livreur_id !== $user->id) return response()->json(['message' => 'Cette commande ne vous est pas assignée'], 403);
    ...
    public function annulerLivraison(Request $request, Commande $commande)
    {
        $user = $request->user();
        if ($user instanceof Livreur && $commande->livreur_id !== $user->id) return response()->json(['message' => 'Cette commande ne vous est pas assignée'], 403);
        if ($commande->statut_livraison !== 'livree') return response()->json(['message' => 'Cette commande n\'est pas marquée comme livrée'], 400);

        $commande->statut_livraison = 'assignee';
        $commande->date_livraison = null;
        $commande->save();

        return response()->json(['message' => 'Livraison annulée, commande remise en cours', 'commande' => $commande->fresh()->load(['client', 'boutique', 'produits'])]);
    }
